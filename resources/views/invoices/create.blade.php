...
@section('page-subtitle', 'Invoice No: ' . $invoiceNo)
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
    ...
    <div class="xl:col-span-2 space-y-5">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <p class="text-sm text-white font-medium">{{ $client->business_name }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">NTN</p>
                    <p class="text-sm text-white font-mono">{{ $client->ntn ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">STRN</p>
                    <p class="text-sm text-white font-mono">{{ $client->strn ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">Province/Authority</p>
                    <p class="text-sm text-emerald-400 font-medium">{{ $client->province }}</p>
                </div>
                <div class="col-span-2">
                    <p class="text-xs text-gray-500">Address</p>
                    <p class="text-sm text-gray-300">{{ $client->address ?? '—' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6">
            ...
            <div id="buyer-details" class="mt-4 grid grid-cols-3 gap-3 hidden">
                <div>
                    <p class="text-xs text-gray-500">NTN</p>
                    <p id="buyer-ntn" class="text-sm text-white font-mono">—</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">Province</p>
                    <p id="buyer-province" class="text-sm text-white">—</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500">Address</p>
                    <p id="buyer-address" class="text-sm text-gray-300">—</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6">
            ...
            <textarea name="notes" rows="3" placeholder="Any additional notes for this invoice..."
                      class="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/30 resize-none"></textarea>
        </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 sticky top-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="text-white font-medium font-mono" id="summary-subtotal">Rs 0.00</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-400">GST Amount</span>
                    <span class="text-white font-medium font-mono" id="summary-gst">Rs 0.00</span>
                </div>
                ...
                <div class="border-t border-gray-700 pt-3 flex justify-between items-baseline">
                    ...
                    <span class="text-emerald-400 text-xl font-bold font-mono" id="summary-total">Rs 0.00</span>
                </div>
            </div>
            ...
            <div class="mt-5 p-3 bg-gray-800/50 rounded-xl border border-gray-700/50">
                ...
                <p class="text-sm text-emerald-400 font-medium">{{ $client->province }}</p>
                <p class="text-xs text-gray-500 mt-1">Default rate: {{ $client->default_gst_rate }}%</p>
            </div>
        </div>
    ...
    row.innerHTML = `
        <div class="col-span-4">
            <select onchange="selectProduct(this, ${idx})"
                ...
                ${products.map(prod => `<option value="${prod.id}" ${p.id == prod.id ? 'selected' : ''} data-price="${prod.price}" data-gst="${prod.gst_rate}" data-hs="${prod.hs_code||''}" data-unit="${prod.unit}">${prod.name}</option>`).join('')}
            </select>
            <input type="text" name="items[${idx}][description]" placeholder="Description *" required value="${p.name||''}"
            ...
            <input type="hidden" name="items[${idx}][product_id]" class="product-id-field" value="${p.id||''}">
        </div>
        <div class="col-span-1">
            <input type="text" name="items[${idx}][hs_code]" placeholder="HS Code" value="${p.hs_code||''}"
        ...
        <div class="col-span-1">
            <input type="number" name="items[${idx}][qty]" value="1" min="0.001" step="0.001" required
        ...
        <div class="col-span-2">
            <input type="number" name="items[${idx}][unit_price]" value="${p.price||0}" min="0" step="0.01" required
                   oninput="recalcRow(this, ${idx})"
